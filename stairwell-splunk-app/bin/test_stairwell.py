...
def test_search_stairwell_hostname_api():
    ...
    res = stairwelllib.stairwellapi.search_stairwell_hostname_api(command, logger, "downloadmoreram.com")

    assert res.get("stairwell_comments_most_recent") == ["Wow", "ok"] 
    assert res.get("stairwell_opinions_most_recent") == ["MALICIOUS"]
    assert res.get("stairwell_ai_assessment") == "blah blah blah"
    assert res.get("stairwell_hostname_a_records", [{}])[0].get("answer") == "10.100.100.100"
    assert res.get("stairwell_hostname_aaaa_records", [{}])[0].get("answer") == "1000:f0b0:1000:c00::00"
    assert res.get("stairwell_hostname_mx_records", [{}])[0].get("answer") == ".com.downloadmoreram.smtp"
